...
const json::Node& JsonReader::GetBaseRequests() const {
    if (!input_.GetRoot().AsDictionary().count("base_requests"s)) return dummy_;
    return input_.GetRoot().AsDictionary().at("base_requests"s);
}

const json::Node& JsonReader::GetStatRequests() const {
    if (!input_.GetRoot().AsDictionary().count("stat_requests"s)) return dummy_;
    return input_.GetRoot().AsDictionary().at("stat_requests"s);
}

const json::Node& JsonReader::GetRenderSettings() const {
    if (!input_.GetRoot().AsDictionary().count("render_settings"s)) return dummy_;
    return input_.GetRoot().AsDictionary().at("render_settings"s);
}

const json::Node& JsonReader::GetRoutingSettings() const {
    if (!input_.GetRoot().AsDictionary().count("routing_settings"s)) return dummy_;
    return input_.GetRoot().AsDictionary().at("routing_settings"s);
}

void JsonReader::ProcessRequests(const json::Node& stat_requests, RequestHandler& rh) const {
    json::Array result;
    for (auto& request : stat_requests.AsArray()) {
        const auto& request_map = request.AsDictionary();
        const auto& type = request_map.at("type"s).AsString();
        if (type == "Stop"s) result.push_back(PrintStop(request_map, rh).AsDictionary());
        if (type == "Bus"s) result.push_back(PrintRoute(request_map, rh).AsDictionary());
...
void JsonReader::FillCatalogue(transport_catalogue::TransportCatalogue& catalogue) {
    ...
    for (auto& request_stops : arr) {
        const auto& request_stops_map = request_stops.AsDictionary();
    ...
    for (auto& request_bus : arr) {
        const auto& request_bus_map = request_bus.AsDictionary();
...
std::tuple<std::string_view, geo::Coordinates, std::map<std::string_view, int>> JsonReader::FillStop(const json::Dictionary& request_map) const {
    ...
    auto& distances = request_map.at("road_distances"s).AsDictionary();
...
std::tuple<std::string_view, std::vector<const transport_catalogue::Stop*>, bool> JsonReader::FillRoute(const json::Dictionary& request_map, transport_catalogue::TransportCatalogue& catalogue) const {
...
renderer::MapRenderer JsonReader::FillRenderSettings(const json::Node& settings) const {
    json::Dictionary request_map = settings.AsDictionary();
...
const json::Node JsonReader::PrintStop(const json::Dictionary& request_map, RequestHandler& rh) const {
    json::Node result;
...
const json::Node JsonReader::PrintMap(const json::Dictionary& request_map, RequestHandler& rh) const {
    json::Node result;
    const int id = request_map.at("id"s).AsInt();
...
const json::Node JsonReader::PrintRouting(const json::Dictionary& request_map, RequestHandler& rh) const {
    json::Node result;
    const int id = request_map.at("id"s).AsInt();
