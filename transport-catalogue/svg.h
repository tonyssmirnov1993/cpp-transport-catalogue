...
namespace svg {
    ...
    protected:
        ...
        void RenderAttrs(std::ostream& output) const {
            ...
            if (fill_color_) {
                output << " fill=\""sv;
                std::visit(ColorPrinter{ output }, *fill_color_);
                output << "\""sv;
            }
            if (stroke_color_) {
                output << " stroke=\""sv;
                std::visit(ColorPrinter{ output }, *stroke_color_);
                output << "\""sv;
            }
            if (width_) {
                output << " stroke-width=\""sv << *width_ << "\""sv;
            }
            if (line_cap_) {
                output << " stroke-linecap=\""sv << *line_cap_ << "\""sv;
            }
            if (line_join_) {
                output << " stroke-linejoin=\""sv << *line_join_ << "\""sv;
            }
        }
